...
AgentMonitoring은 먼저 `origin`을 fetch해 원격 기준 브랜치를 확인해요. 원격에 새 커밋이 생겼다면 작업 브랜치만 최신 원격 위로 옮기고 프로젝트 테스트, Simulator 검증과 Reviewer를 다시 실행해요. 재검증이 끝나면 변경 내용을 다시 확인하고 한 번 더 게시하세요. 충돌이나 재검증 실패가 발생하면 원격과 로컬 원본은 그대로 유지돼요.

PR 방식에서는 PR을 만든 뒤 작업이 **PR 병합 대기**가 돼요. GitHub에서 병합하고 **PR 상태 확인**을 누르면 원격 반영을 확인한 뒤 로컬 기준 브랜치를 fast-forward해요. 직접 게시 방식은 강제 push 없이 원격 기준 브랜치가 예상한 commit일 때만 게시해요.
