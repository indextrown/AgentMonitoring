...
7. Reviewer가 문제를 찾으면 보고서를 바탕으로 다시 수정하고 검증해요.
...
| 결과와 자가 수정 | 프로젝트 테스트, 화면·접근성·Debug 상태 수집, 합격 조건 판정, 테스트·실행 실패와 Reviewer finding을 이용한 제한된 재시도 |
...
3. 최초 구현을 포함한 최대 구현 시도 횟수를 정하세요.
