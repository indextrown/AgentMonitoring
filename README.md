...
4. 선택한 경우에만 Test Designer와 Critic이 테스트 범위를 설계하고 검토해요.
5. Implementer가 별도 Git worktree에서 코드를 수정해요.
6. 선택한 프로젝트 테스트와 실제 앱 동작을 검증해요.
7. 실패하면 로그와 화면 증거를 바탕으로 다시 수정해요.
8. Reviewer가 문제를 찾으면 보고서를 바탕으로 다시 수정하고 검증해요.
9. 사람이 승인하면 변경을 현재 로컬 브랜치에 적용해요.
...
| --- | --- | --- |
| 프로젝트 테스트만 | 로직, 데이터 처리, 회귀 수정 | 선택한 경우 테스트 설계·비평 → 구현 → 등록한 검증 명령 → Reviewer |
| Simulator 검증만 | 화면 표시와 사용자 조작이 핵심인 Swift 앱 작업 | 구현 → iPhone·iPad Simulator 실행·조작·판정 → Reviewer |
| 프로젝트 테스트 + Simulator | 로직과 화면 동작을 함께 바꾸는 기능 | 테스트 설계·비평 → 구현 → 프로젝트 테스트 → Simulator 검증 → Reviewer |
...
작업 상세에서는 테스트 설계, 프로젝트 테스트, Simulator와 Reviewer 결과를 각각 **통과**, **확인 필요**, **건너뜀**으로 표시해요. 수동 검토 작업은 자동 통과로 표시하지 않아요.
...
| 결과와 자가 수정 | 단계별 통과·실패·건너뜀 표시, 프로젝트 테스트, 화면·접근성·Debug 상태 수집, 합격 조건 판정, 제한된 재시도 |
...
4. 프로젝트 테스트를 사용할 계획이면 추천 검증 명령을 선택하거나 **프로젝트 설정**에서 직접 입력하세요.

![프로젝트 준비 상태](./tests/e2e/dashboard.spec.ts-snapshots/project-readiness-chromium-desktop-darwin.png)

검증 명령은 프로젝트 테스트를 선택한 작업의 기본 테스트예요. 예를 들면 `pnpm test`, `swift test`, `tuist test`나 `xcodebuild ... test`를 등록할 수 있어요. 검증 명령이 없어도 Simulator 검증만 또는 수동 검토만 선택한 작업은 시작할 수 있어요.
...
승인하거나 폐기한 작업의 격리 worktree와 DerivedData는 바로 지워요. 화면 캡처와 접근성 트리 같은 실행 증거는 아래 보관 정책에 따라 남겨 검토할 수 있어요. 실패하거나 중단한 작업은 다시 실행할 수 있도록 작업공간과 빌드 산출물을 유지하며, 더 이상 필요하지 않으면 작업 상세에서 **폐기**할 수 있어요.
