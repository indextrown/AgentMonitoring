...
```text
Code → Build → Run → Observe → Act → Verify → 사람 승인
```
...
10. 사람이 승인하면 변경을 현재 로컬 브랜치에 적용해요.
...
| 소스 제어 | 원본 저장소의 변경 파일과 diff 확인, 파일별 스테이징·해제, staged 파일만 커밋, 모든 변경 한 번에 커밋 |
| 검토와 적용 | 변경 파일·Git patch·Reviewer finding·시도별 증거 확인, 외부 IDE로 worktree 열기, 최신 원본을 반영·재검증한 뒤 로컬 브랜치에 적용 |
...
결과가 맞으면 **원본에 적용**을 누르세요. 원본 브랜치가 작업 시작 이후 바뀌지 않았다면 앱이 작업 브랜치를 커밋하고 바로 적용해요.

원본 브랜치에 새 커밋이 생겼다면 작업 브랜치만 최신 원본 위로 옮겨요. 원본 checkout은 건드리지 않은 채 프로젝트 테스트, Simulator 검증과 Reviewer를 다시 실행해요. 재검증이 끝나면 변경 내용을 다시 확인하고 한 번 더 적용하세요. 충돌이나 재검증 실패가 발생하면 원본은 그대로 유지돼요.

## 앱 안에서 변경을 커밋하려면

원본 checkout에 커밋하지 않은 변경이 있으면 AI 작업을 원본에 적용할 수 없어요. 왼쪽 **소스 제어**에서 변경을 정리하세요.
...
커밋 뒤에도 다른 파일을 남겨 둘 수 있어요. 다만 AI 작업을 **원본에 적용**하려면 원본 checkout의 모든 변경을 먼저 커밋해야 해요. AgentMonitoring은 파일 폐기, amend, push와 충돌 해결은 수행하지 않아요.
...
에이전트는 commit, push, merge나 배포를 수행하지 않아요. 사용자가 **원본에 적용**을 누른 뒤에만 AgentMonitoring이 작업 변경을 로컬 커밋으로 만들어요. 원본이 앞서 있으면 작업 브랜치만 재배치하고 검증을 다시 실행해요. 원격 push와 PR 생성은 사용자가 별도로 진행해야 해요.
