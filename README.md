...
1. Codex가 작업과 저장소에 맞는 검증 조합을 추천해요.
2. 사람이 프로젝트 테스트, Simulator 검증 또는 수동 검토 중 필요한 방식을 선택해요.
3. Simulator 검증을 선택하면 Codex가 조작할 동작과 합격 조건을 제안해요.
4. 새 Git worktree에 필요한 의존성과 도구 환경을 준비해요.
5. 선택한 경우에만 Test Designer와 Critic이 테스트 범위를 설계하고 검토해요.
6. Implementer가 별도 Git worktree에서 코드를 수정해요.
7. 의존성 설정이 바뀌었다면 환경을 다시 준비하고, 선택한 프로젝트 테스트와 실제 앱 동작을 검증해요.
8. 코드 검증이 실패하면 로그와 화면 증거를 바탕으로 다시 수정해요.
9. Reviewer가 문제를 찾으면 보고서를 바탕으로 다시 수정하고 검증해요.
10. 사람이 승인하면 작업에서 선택한 방식으로 원격에 게시해요. 작업 브랜치를 올리고 PR을 만들거나, 기본 브랜치에 직접 push할 수 있어요.
11. 원격 반영이 확인되면 현재 로컬 브랜치를 fast-forward로 동기화하고 완료해요.
...
로컬 Git 프로젝트 연결
  ...
  → 작업 브랜치 push·PR 생성 또는 기본 브랜치 직접 push
...
| 에이전트 실행 | AI 검증 계획 추천, 작업별 조건부 파이프라인, Test Designer·Critic·Implementer·Reviewer 역할 분리, 작업별 worktree |
...
| 소스 제어 | 원본 저장소의 변경 파일과 diff 확인, 파일별 스테이징·해제, staged 파일만 커밋, 모든 변경 한 번에 커밋, origin fetch와 ahead·behind 확인 |
| 검토와 게시 | 변경 파일·Git patch·Reviewer finding·시도별 증거 확인, 최신 원격 반영·재검증, 작업 브랜치 push·GitHub PR 생성 또는 기본 브랜치 직접 push, 로컬 fast-forward 동기화 |
...
3. **AI에게 추천받기**를 누르고 검증 조합과 테스트 설계 방식을 확인하세요.
4. 최초 구현을 포함한 최대 구현 시도 횟수를 정하세요.
5. 이 작업 전용 Simulator 시나리오를 선택했다면 **검증 시나리오 만들기**를 누르세요.
6. Codex가 제안한 accessibility identifier, 입력값과 예상 결과를 확인하세요.
7. **검증 계획 확인하고 작업 등록**을 누르세요.
...
결과가 맞으면 작업에서 선택한 **브랜치 올리고 PR 만들기** 또는 **기본 브랜치에 직접 게시**를 누르세요. 프로젝트 설정에서 기본 방식을 정하고, 새 작업을 만들 때마다 바꿀 수 있어요.
...
커밋 뒤에도 다른 파일을 남겨 둘 수 있어요. 다만 AI 작업을 게시하려면 원본 checkout의 모든 변경을 먼저 커밋해야 해요. 로컬 기준 브랜치에 원격에 없는 커밋이 있으면 그 커밋을 별도로 게시한 뒤 계속해야 해요. AgentMonitoring은 파일 폐기, amend와 충돌 자동 해결은 수행하지 않아요.
...
에이전트 자체는 commit, push, merge나 배포를 수행하지 않아요. 사람이 최종 결과와 게시 방식을 승인한 뒤에만 호스트 앱이 작업 변경을 커밋하고 원격 상태를 확인해요. 원격이 앞서 있으면 작업 브랜치만 재배치하고 검증을 다시 실행해요. 이후 작업 브랜치와 GitHub PR을 만들거나 기본 브랜치에 fast-forward push하고, 원격 반영을 확인한 뒤 로컬을 동기화해요. 강제 push와 자동 PR merge는 하지 않아요.
