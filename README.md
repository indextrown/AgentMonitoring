...
11. 승인 대기 중 새 문제를 발견하면 추가 수정 요청을 남겨 같은 작업공간에서 구현과 검증을 다시 진행해요.
...
로컬 Git 프로젝트 연결
  ...
  → 선택한 프로젝트 테스트와/또는 Simulator 실행·조작·판정
      ...
      └─ 통과: Reviewer 검토
          ...
          └─ 문제 없음: 사람의 최종 승인 대기
  → 변경 파일·테스트·실행 증거 확인
      ├─ 추가 문제 발견: 같은 작업에 수정 요청 → 테스트·구현·검증 반복
      └─ 문제 없음: 게시 승인
  → 사람 승인
...
테스트와 Reviewer가 통과했더라도 작업본을 직접 실행하다가 경고나 빠진 동작을 발견할 수 있어요. 이때 새 작업을 만들 필요가 없어요. 승인 대기 화면의 **추가 수정 요청**에 문제와 기대 결과를 적고 **요청하고 작업 이어가기**를 누르세요.
...
추가 요청은 여러 번 남길 수 있으며 이전 요청도 함께 전달돼요. 이미 원격 게시가 시작됐거나 원격 반영 후 로컬 동기화만 남은 작업은 변경하지 않아요.
...
| 작업 관리 | 새 작업 등록, 선택형 테크스펙 생성·수정·승인, 승인 대기 작업의 추가 수정 요청과 이력, 실시간 상태와 역할별 로그, 중단·재실행, 검색, 버그·메모·활동 기록 |
