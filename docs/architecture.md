...
| 상태 | 의미 |
| --- | --- |
| `ready` | 현재 AgentMonitoring이 바로 사용할 수 있다. Code, 저장된 검증 명령, 유효한 iOS 계약의 Build·Run·Observe screen·accessibility가 해당한다. |
| `declared` | 프로젝트 계약에는 있지만 실행 adapter가 아직 연결되지 않았다. |
| `missing` | 프로젝트에서 선언하거나 설정하지 않았다. |

유효한 iOS 계약에서 `build`와 `run`이 활성화되면 AgentRunner가 테스트 통과 뒤 Xcode와 Simulator runtime을 시작한다. `observe`의 `screen`은 화면 캡처 adapter에, `accessibility`는 XCTest snapshot observer에 연결한다. 상태 관찰과 `act`, runtime `verify`는 아직 선언 상태로 표시한다. manifest가 없거나 Build·Run이 비활성화된 프로젝트는 기존 코드 작업 모드로 동작한다.
...
| Swift Runtime | 직접 실행 | 없음 | worktree 앱 빌드, 선택한 iPad·iPhone Simulator 설치·실행, 선언된 화면·접근성 증거 수집 |
...
원본 프로젝트의 선언형 계약 읽기
  ...
  → Observe accessibility 계약이면 번들된 UI-test observer 실행
  → `XCUIApplication.snapshot()` 계층을 JSON으로 저장하고 Reviewer 프롬프트에 전달
```
...
접근성 observer는 패키지에 포함된 Xcode UI-test target을 session으로 복사한 뒤 실제 bundle identifier를 테스트 plan에 주입해 실행한다. 프로젝트에 UI-test target을 추가하지 않고 앱 코드도 수정하지 않는다. identifier, label, title, value, placeholder, frame, enabled·selected 상태와 children을 최대 5,000개 요소·64단계·512KB로 제한한다. marker 사이 base64 payload만 허용하고 JSON schema와 bundle identifier를 다시 검증한 뒤 최대 1MB JSON 증거로 저장한다. Reviewer 입력은 60,000자로 제한하고 전체 파일은 로컬 session에 보존한다.

runtime session은 `preparing`, `booting`, `building`, `installing`, `launching`, `observing`, `running`, `failed`, `stopped` 상태를 가지며 기기 UDID·이름, bundle identifier, PID와 마지막 진단을 저장한다. 화면·접근성 증거 메타데이터는 별도 레코드로 보존해 작업 상세에서 파일을 열 수 있다.
...
| `runtime_evidence` | 작업별 화면·접근성 증거의 로컬 경로, MIME type, 크기와 생성 시각 |
...
- Xcode container는 worktree 내부 실경로, `.app` 산출물은 작업 전용 DerivedData 내부 실경로, 화면 PNG와 접근성 JSON은 작업 전용 evidence 내부 실경로일 때만 허용한다.
- 화면 증거는 Observe screen 계약에서 Reviewer의 `--image` 입력으로, 접근성 JSON은 Observe accessibility 계약에서 제한된 프롬프트 문맥으로만 전달한다.
...
- Simulator Debug 상태 관찰, UI·fixture 조작, runtime 시나리오 자가수정은 아직 없다.
