...
`PlanningRepositoryContext`는 Git HEAD·브랜치·porcelain 상태와 변경 파일의 크기·mtime·ctime으로 변경을 확인한다. 같은 지문이면 메모리에 저장한 파일 목록을 재사용하고 달라지면 다시 수집한다. 메모리 캐시는 최대 20개 저장소로 제한한다. 파일 본문·비밀값은 캐시하지 않으며 초기 후보 목록에서 의존성·산출물·비밀 파일 경로를 제외한다. 최대 24개 초기 조사 후보를 제공한다. 프롬프트는 관련 파일 6개·파일당 160줄·검색 3회를 초기 조사 지침으로 제시한다. 이 수치는 도구 권한의 강제 제한이 아니며 서버 프롬프트 캐시와도 별개다.
...
`turn/start` 응답 후 45초가 지나거나 도구 시작 이벤트가 8개가 되어도 조사 도구가 실행 중이면 `turn/steer`를 한 번 보내 초안 작성을 우선하도록 요청한다. 이미 Markdown을 수신했거나 도구 실행을 마쳤으면 보내지 않는다. 작성 중인 응답 뒤에 추가 답변이 예약되는 지연을 피하기 위한 조건이다. 전환 요청은 같은 turn에 입력을 추가하는 동작이며 새 생성 turn이나 자동 재시도가 아니다. 미확인 내용은 본문에 명시하고 원본 요구사항·사용자 결정을 유지하도록 지시한다. 전환 요청이 실패해도 중복 생성하지 않고 UI에 알린다. 진행 이벤트가 90초 동안 없으면 별도 무응답 오류로 중단한다. 진행 이벤트가 계속 와도 전체 3분 제한은 연장하지 않는다. 두 제한 모두 `turn/interrupt`와 연결 종료로 정리한다.

`PlanningDiagnostics`는 앱 데이터 폴더의 `planning-diagnostics/tech-spec.jsonl`에 최근 50회 기록을 직렬·원자적으로 저장한다. 허용 필드는 요청 ID, 시작 시각·소요 시간, 결과 분류, 마지막 단계, 도구 호출 수, 초안 전환 요청 여부, 첫 미리보기 시간, 대화·파일 목록 재사용 여부다. JSON 검증 실패도 실패로 기록하며 요구사항·코드·응답·원시 오류·인증값은 기록하지 않는다. 인증·모델 조회 실패는 생성기 진입 전이므로 이 진단 파일에 포함하지 않는다. 파일 저장 실패가 생성 결과를 실패로 바꾸지는 않는다.
