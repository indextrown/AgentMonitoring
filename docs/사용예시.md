...
이 예시의 목표는 완료 카드에 빠진 `completion-card` 접근성 식별자를 Codex가 추가하고, 프로젝트 테스트와 runtime assertion을 모두 통과시키는 거예요. 실제 실행에서는 Test Designer가 실패 조건을 만들고, Implementer가 코드를 수정하며, AgentMonitoring이 Simulator 화면과 접근성 트리를 증거로 남겨요.
