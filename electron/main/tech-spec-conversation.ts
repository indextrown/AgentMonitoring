...
export class TechSpecConversations {
  ...
  constructor(private readonly command: string, private readonly codexHome?: string, private readonly timeoutMs = 180_000, private readonly idleMs = 60_000) {}
  ...
  async run(input: PlanningRequest & { instructions: string; requirements: string; model?: CodexModelSelection; outputSchema: object }): Promise<string> {
    ...
    const timer = setTimeout(() => controller.abort(new Error('테크스펙 생성이 3분 안에 끝나지 않았습니다. 초안과 의견은 보존됩니다.')), this.timeoutMs)
    const active = this.execute(draft, { ...input, signal: controller.signal })
    draft.active = active
  ...
  private async execute(draft: DraftSession, input: PlanningRequest & { instructions: string; requirements: string; model?: CodexModelSelection; outputSchema: object }): Promise<string> {
    ...
    try {
      ...
      const receive = (message: RpcObject): void => {
        ...
        if (message.method === 'item/agentMessage/delta' && typeof params.delta === 'string') {
          ...
          publish({ stage: 'writing', message: '테크스펙 작성 중', preview: markdownPreview(text) || progress.preview }, false)
        }
        if (message.method === 'item/started' && params.item?.type === 'commandExecution') {
          ...
          publish({ stage: 'investigating', message: paths.length ? `관련 파일 확인: ${paths.slice(0, 3).join(', ')}` : '관련 코드 읽기·검색 중' })
        }
